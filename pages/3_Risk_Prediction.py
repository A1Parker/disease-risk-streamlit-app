...
with col1:
    ...
    daily_steps = st.number_input("Daily Steps", 0, 30000, value=6000)
    stress_level = st.number_input("Stress Level (1-10)", 1, 10, value=5)
    sleep_hours = st.number_input("Sleep Hours", 0.0, 12.0, value=7.0)
    physical_activity = st.number_input("Physical Activity (Minutes)", 0, 400, value=45)
    cholesterol = st.number_input("Cholesterol Level", 100, 400, value=180)
    glucose = st.number_input("Glucose Level", 50, 300, value=95)
    heart_rate = st.number_input("Heart Rate", 40, 150, value=72)
...
with col2:
    ...
    if probability > 50:
        ...
        if cholesterol > 240:
            recommendations.append("Monitor cholesterol levels and improve diet.")
        ...
        if recommendations:
            ...
            st.warning("General lifestyle improvement recommended.")
    ...
    # Low Risk
        ...
        if positive_points:
            ...
            st.success("You are currently at low risk. Maintain a balanced lifestyle.")
